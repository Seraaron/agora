...
'Kismet' here mainly refers to **spending [[Lots]] and [[Ego]]**. However, for extraordinary rolls, [[Health]] and [[Fidelity]] may also be used here _while KISSing_ for additional special effects:

- **Spending a [[Lots|Lot]] lets you _re-roll a die_**, counting as a KISS _with no questions asked_.
	- There's no limit to how many Lots you can spend, except:
		- You only have eight _at most_ at any one time, and
		- You may not KISS more dice than you initially rolled.
- **Spending [[Ego]] doubles the successes counted from all 6s rolled**.
	- *If your dice are  [[Exploding Dice|exploding]]*, then spending Ego will instead let you **roll your current [[Health]] as additional bonus dice**.
	- These bonus dice can't be KISSed, but they _can_ also explode.
...
**Each roll can only be KISSed by a single stunt**, at most.
...
❗ _Stunts don't need Peer approval_ in the same way that Insights or Specialties do.
...
Furthermore, when there is an [[Opposed Tasks|Opposed Task]] or a [[Versus Checks|Vs. Check]], these restrictions prevent one side from re-rolling first, and then the other side using that information to decide how much to KISS.

❓ Generally, **there's no point in KISSing a die that has already succeeded** — unless you're desperately trying to roll 6s to use [[Ego]] or cause an [[Exploding Dice|explosion]] — but if you do so and the second result is worse then you must keep it.

#### Helpers can KISS for you
...
❗ Be careful not to 'double-dip': Meaning, if two or more things are _applicable but similarly-veined_, then only take one KISS.
